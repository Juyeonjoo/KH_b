...
WHERE
   last_name=initcap('KING');       --값에다가 함수를 적용 
   --실제 테이블 안에 이름이 어떤형태로 되어있는지 미리 알아야 함
   --컬럼에다가 upper함수를 적용하는게 아니라, 값에다가 적용하게 하는 것
   --동일한 결과를 낼 수 있도록 
...
SELECT
   -- || (Concatenation Operator) == concat function
   last_name || salary,
   concat(last_name, salary) --문자열을 데이터로 갖고있는 컬럼 AS
FROM
   employees;
...
SELECT
   ...
   to_char(hire_date, 'RR/MM/DD')         AS 일사일2,   --문자열로 강제형변환시킨 것.TO_CHAR
