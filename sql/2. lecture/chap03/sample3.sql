...
ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD';     --SQL developer
ALTER SESSION SET NLS_DATE_FORMAT = 'DD-MON-DD';    --vscode
...
SELECT
    ...
    -- current_date * 3    --(x)오류,ORA-00932: inconsistent datatypes: expected NUMBER got DATE
   urrent_date / 3, 
    CURRENT_DATE  - 1 AS 어제,           -- 현재날짜 - 하루
    current_date - (current_date -1),    --차이가 발생한 "일수"
    current_date + 3/24,                 --3시간을 더하는 것.
    current_date + 3/24/60,              --3분을 더하는 것.
    current_date + 3/24/60/60,           --3초 더하는 것
...
ORDER BY
근속일수 DESC;
    -- 3 DESC;   --sort by column index 이렇게 쓰지 말기
