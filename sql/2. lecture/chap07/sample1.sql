...
-- ------------------------------------------------------
 -- 0. 자동 커밋(Auto-Commit)이란?
 -- 커밋(COMMIT) : DML 문장에 의한 DB 의 변경 내역을 영구적으로 DB 파일에 저장하는 기능
 -- 롤백(ROLLBACK) : INSERT문에 의한 변경이 발생하기 이전 상태로 회귀시킴
 --  DML1개 문장이 실행완료될 때마다,자동으로 COMMIT 문을 수행시키는 기능
-- ------------------------------------------------------
SHOW AUTOCOMMIT;            --현재 DB 연결 세션에서, 자동커밋 설정을 보여줌
SET AUTOCOMMIT ON;         -- 현재 DB 연결 세션에 대해 자동커밋을 ON 시킴
SET AUTOCOMMIT OFF;       -- 현재 DB 연결 세션에 대해서 자동 커밋을 OFF 시킴
-- ------------------------------------------------------
-- 1-1. 단일 행 INSERT 문
-- ------------------------------------------------------
...
BEGIN       -- To begin a transaction
    ...
    -- ROLLBACK;
    -- 5. TCL: COMMINT 또는 ROLLBACK문으로 구성됨
    COMMIT;             --***영구적으로 위의 INSERT 문에 의한 변경을 저장
    -- ROLLBACK;        -- ***위의 INSERT문에 의한 변경이 발생하기 이전 상태로 회귀시킴
...
-- ------------------------------------------------------
-- * CTAS: 기존 테이블 스키마 복사 시, NOT NULL 제약조건((null, pk, fk, unique))을 제외
--         한, 그 외 제약조건은 복사되지 않음.
-- ------------------------------------------------------
-- Basic Syntax) 
--
...
--  Subquery;
-- ------------------------------------------------------
DROP TABLE mydept;          --DDL (Data Definition Langauge) : 테이블 삭제 구문
