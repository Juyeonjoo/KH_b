...
-- 다. check조건을 WHERE 절에 기재(조인조건과 검색조건을 분리)
-- 라. 가독성 향상
-- ------------------------------------------------------
...
FROM
    ...
    --employees, departments in Oracle Catesian
...
SELECT
    ...
    -- ERROR at line 95:ORA-00904: "EMPLOYEES"."LAST_NAME": invalid identifier
...
SELECT
    ...
    -- employees.DEPARTMENT_ID  -- XX 공통컬럼은 수식어(Qualifier)를 가질 수 없다.
-- t1.department_id    --XX: 공통컬럼은 수식어(Qualifier)를 가질 수 없다.
...
FROM
    ...
    -- USING절의 한계 : 무조건 Equi Join -> Non-Equi Join 수행 불가
...
WHERE
    t1.department_id = 90;                      -- check조건(가독성 저하)
...
DEPARTMENT_ID AS 부서번호           --ok
    -- D.DEPARTMENT_ID AS 부서번호     --xx  ORA-25154: column part of USING clause cannot have qualifier
