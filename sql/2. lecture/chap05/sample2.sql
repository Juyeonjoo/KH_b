...
-- A. Catesian Product (카테시안 프로덕트)
-- ------------------------------------------------------
-- 두 개 이상의 테이블을 조인조건(공통컬럼x)없이 연결하는 조인으로,
...
FROM
    dual;    
--Cartesin Product Operation 

SELECT
    -- count(*)        --2889 = 107 x 27
    *
FROM
--연결시킬 여러 테이블을 아래와 같이 쉼표(,)로 나열하면됨
--조인조건 없음(***)
    employees,
    departments;
...
SELECT
    last_name,  --사원테이블에만 있다 employees
    department_name --사원테이블에 없고 departments에 있다.
    --원하는 정보가 두 테이블에 걸쳐서 있다.
FROM
    employees,
    departments;
...
-- 조인 조건(동등조건) 기재
-- 왼쪽, 자식테이블의 FK            오른쪽,부모테이블의 PK
    employees.department_id = departments.department_id; --조인 조건,(체크조건 아님)
-- 이 두개가 동등비교 연산자로 이어져야 > 조인
...
-- 자연 조건(nature join) == Equijoin+ 중복제거
FROM
    employees,
    departments
WHERE
...
SELECT
    ...
    -- employees.department_id       -- 부서번호 xxx
emp.DEPARTMENT_ID

FROM
--한번 테이블에 별칭(Alias)를 보여하면, DQL문장을 구성하는
--모든 저렝서 테이블명이 나올 필요가 있는 곳에는 모두,
-- 테이블 별칭을 사용해야 함.
...
WHERE
--ORA-00904: "EMPLOYEES"."DEPARTMENT_ID": invalid identifier
    -- employees.department_id = dept.department_id; --alias사용안함
e.department_id = d.department_id;  -- 조인조건

-- ------------------------------------------------------
--Check조건(1차 필터링을 수행하는 조건/검색조건이라고도함) 추가
-- ------------------------------------------------------
...
-- 라. 결과: 조인조건의 결과 중에서, Check조건으로 필터링 된 결과
--          를 반환
-- ------------------------------------------------------
SELECT
    e.last_name,
...
-- 2개의 테이블을 비동등조인(non-EquiJoin ) 수행
...
--첫 번째 쿼리 : 나를 관리하는 관리자 사번 뽑기
SELECT
    employee_id,    --사원 번호(사번)
    last_name,       -- 나의 이름
    manager_id     -- 관리자 사번
...
-- ------------------------------------------------------
-- INNER 조인의 조건에 부합하지 못해, 탈락한 행들을 되살리는 조인
...
SELECT
    ...
    m.last_name AS 관리자명
FROM
-- 지시하는 자와 지시 받는자의 정보를 획득 => self join필요
    employees e,        -- 사원 정보
    employees m         -- 관리자 정보(가상)
WHERE
    --동등 조인 조건을 생성 
    -- FK(자식) --   --PK(부모)-- 
    e.manager_id = m.employee_id       --회장님은 manager가 없어서 null값, 탈락
ORDER BY
    e.employee_id;
...
FROM
    ...
    employees m         -- 관리자 정보(가상)
WHERE
    e.manager_id = m.employee_id (+) --왼쪽을 살림.
    --  e.manager_id (+) = m.employee_id  --오른쪽을 살림.
ORDER BY
    e.employee_id;
