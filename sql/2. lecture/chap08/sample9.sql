...
-- Oracle은 기존 제약 조건을 수정하려면 => (1) 삭제 (2) 추가하는 방식으로 함
--기존 제약 조건을 이름으로 지정하여 삭제
ALTER TABLE emp02 DROP CONSTRAINT emp02_deptno_fk;
--기존 제약 조건을 이름으로 수정
ALTER TABLE emp02 ADD CONSTRAINTS emp02_deptno_fk FOREIGN KEY(deptno) REFERENCES dept02(deptno)
ON DELETE SET NULL;