...
SELECT
    *
FROM
    HR.employees;       --OK: HR계정 밑의 이 테이블에 대한 SELECT 권한을 받았음.
...
--          우선 할당받아야 함.
-- (*주의2*) PUBLIC 동의어를 만들고 싶을 때에는 SYS계쩡(클라우드는 ADIN)
--      으로 부터, CREATE PUBLIC SYNONYM 시스템 권한을 할당받아야 한다. 
-- (*주의3*) 동의어를 사용할 수 있다라는 문제와, 다른 계정 소유의 DB 객체에 대한
--  접근 권한의 문제는 각각 다른 문제이다.
-- ------------------------------------------------------

-- SYS 계정에서 수행
GRANT CREATE SYNONYM TO scott;
GRANT CREATE PUBLIC SYNONYM TO scott; --public synonym 생성권한


-- SCOTT 계정
CREATE SYNONYM emp_synonym      --Private 동의어
FOR hr.employees;       

CREATE PUBLIC SYNONYM emp_synonym         --Public  생성
FOR hr.employees;

CREATE PUBLIC SYNONYM p_emp       --Public  생성, for 에 대해서 동의어로 p_emp 사용하겠다.
FOR HR.employees;                  --동의어 사용 가능일뿐, employees에 대한 접근 권한은 없다.!!!
