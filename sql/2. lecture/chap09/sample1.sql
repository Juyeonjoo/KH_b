...
--    b. 정렬을 위한 ORDER BY 절은 사용불가 (틀림, 사용 가능)
--    c. 정렬 필요시, View 이용 검색쿼리에서 기술해야 함 (틀림, 사용 가능)
--  * View 의 수정:
...
FROM
    ...
    departments t2
WHERE
    t1.department_id = t2.department_id             -- 조인조건(동등조인, EquiJoin)
GROUP BY
    t2.department_name;
...
WHERE
    deptno = 30
WITH CHECK OPTION;      -- 제약 조건 추가하여, 뷰 수정
...
-->>뷰에 대한 DML 작업이, 뷰를 만든 서브쿼리의 WHERE절의 조건을
--          만족(true) 시키지 못하면(false), 그 DML 작업은 불가능하다.
...
-- 이런 DML작업을 제약하는 것이 바로 with check option 제약 조건
   -- deptno = 40,        -- XX: WITH CHECK OPTION 제약조건위배
    sal = 10000         -- OK
WHERE
    empno = 7521;
