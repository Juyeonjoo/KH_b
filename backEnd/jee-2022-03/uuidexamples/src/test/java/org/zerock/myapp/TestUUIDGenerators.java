...
//	@AfterEach
//	void afterEach() {	//매 테스트 케이스마다 수행하는 후처리
...
//	@Disabled
	...
	void type3UUID() throws UnsupportedEncodingException {
		...
		//Type3 UUID는 이름공간(namesapce)과 이 공간 안에 있는
		//특정 이름(name)기반으로 UUID 를 생성하는 방법.
		
		//규칙1: namespace의 길이는 반드시 32문자 길이어야 한다.
...
//	@Disabled
	@Test
	@Order(4)
	@DisplayName("테스트5: Type4 UUID 생성")
	@Timeout(value=500, unit=TimeUnit.MILLISECONDS)
	void type5UUID() throws UnsupportedEncodingException {
		log.trace("type5UUID() invoked.");
		...
		//Junit에서는 assertNotNull을 쓰는게 일반적이다.
...
//	@Disabled
	@Test
	@Order(5)
	@DisplayName("테스트5: UUID와 Message Digest 알고리즘을 이	용한 Unique Key 생성")
	@Timeout(value=200, unit=TimeUnit.MILLISECONDS)
	void typeUUIDAndMessageDigest() throws NoSuchAlgorithmException, UnsupportedEncodingException{
		log.trace("typeUUIDAndMessageDigest() invoked.");
		
		String uuid = UUIDGenerator.generateUniqueKeysWithUUIDAndMessageDigest();
		
		Objects.requireNonNull(uuid);
		log.info("\t+uuid:{}",uuid);
	}//typeUUIDAndMessageDigest
